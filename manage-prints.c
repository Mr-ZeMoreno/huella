...
main (void)
{
  ...
  list_data->_fingerprint._clear_storage._session.ret_value = EXIT_FAILURE;
  list_data->_fingerprint._clear_storage._session.loop = g_main_loop_new (NULL, FALSE);

  fp_device_open (dev, NULL, (GAsyncReadyCallback) on_device_opened, list_data);

  g_main_loop_run (list_data->_fingerprint._clear_storage._session.loop);

  return list_data->_fingerprint._clear_storage._session.ret_value;
}
