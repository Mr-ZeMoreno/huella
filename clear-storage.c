                    ...
                    ClearStorageData *clear_storage_data)
{
  if (!fp_device_is_open (dev))
    {
      g_main_loop_quit (clear_storage_data->_session.loop);
      return;
    }

  fp_device_close (dev, NULL, (GAsyncReadyCallback) clear_storage_device_closed,
                   clear_storage_data);
}
      ...
      if (!clear_saved_prints (dev, &error))
        {
          g_warning ("Clear saved prints from local storage failed: %s",
                     error->message);
          clear_storage_data->_session.ret_value = EXIT_FAILURE;
        }
      else
        {
          g_print ("Clear storage successful!\n");
          clear_storage_data->_session.ret_value = EXIT_SUCCESS;
        }
...
main (void)
{
  ...
  clear_storage_data->_session.ret_value = EXIT_FAILURE;
  clear_storage_data->_session.loop = g_main_loop_new (NULL, FALSE);
  ...
  g_main_loop_run (clear_storage_data->_session.loop);

  return clear_storage_data->_session.ret_value;
}
